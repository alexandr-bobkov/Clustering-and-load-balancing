...
```
#### Конфигурационный файл haproxy (/etc/haproxy/haproxy.cfg) был отредактирован для работы на 4 уровне (TCP) с использованием алгоритма Round-robin. Балансировщик принимает запросы на порту 8888 (так как порт 80 занят Nginx).
...
#### Запустим проверку 
```bash 
for i in {1..4}; do curl http://192.168.1.129:8888; done
``` 
...
## Как работает эта связка c keepalived:
    ...
    Мониторинг: Keepalived на «Мастере» постоянно проверяет (актуально если 2 и более сервера с HAProxy): «Работает ли мой локальный HAProxy?».
    Реакция: Если скрипт возвращает ошибку (HAProxy упал), Keepalived понимает, что этот сервер больше не годен для работы.
    Переезд IP: Он передает  Virtual IP второму серверу (Backup) c HAProxy.
    Результат: Пользователи продолжают заходить на тот же IP, но попадают уже на второй HAProxy, который «живой».
